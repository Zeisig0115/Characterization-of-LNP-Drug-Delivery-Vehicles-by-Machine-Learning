...
def augment_type_data(names, X, new_sample_count=2000):
    ...
    # 若直径数据需要浮点处理，可以直接使用 diameters（例如20.0, 21.0, ..., 100.0）
...
def add_noise_to_curve(curve, q=None):
    ...
    return curve_noisy / normalization
...
def generate_heter_data(X0, X1, desired_count=1000):
    ...
    return np.array(X_heter_list), np.array(alpha_list, dtype=np.float32)


def load_hdf5_data_for_regression(file_type3, file_type4, desired_count=2000, heter_count=1000):
    """
    ...
    # 生成 heterogeneous 数据：随机从扩增后的 X0 与 X1 中采样混合生成新曲线，返回混合比例 alpha
    X_heter, alphas = generate_heter_data(X0, X1, desired_count=heter_count)
...
if __name__ == "__main__":
    ...
    desired_count = 10000
    heter_count = 20000
    X, y = load_hdf5_data_for_regression(file_type3, file_type4, desired_count, heter_count)
    ...
    model = Resnet(input_length=X.shape[1]).to(device)
